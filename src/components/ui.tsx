...
export function Badge({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <span className={cn('inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium border', className)}>
      {children}
    </span>
  );
}
...
export function Tooltip({ text, children }: { text: string; children: ReactNode }) {
  ...
  return (
    <div className="relative inline-flex" onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}>
      {children}
      {show && (
        <div className="absolute z-50 bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-[#1a1a2e] text-white/80 text-[11px] rounded-lg shadow-xl whitespace-nowrap max-w-xs text-center leading-relaxed border border-white/[0.06]">
          {text}
          <div className="absolute top-full left-1/2 -translate-x-1/2 -mt-1 w-2 h-2 bg-[#1a1a2e] rotate-45 border-r border-b border-white/[0.06]" />
        </div>
      )}
    </div>
  );
...
export function HelpBadge({ text }: { text: string }) {
  return (
    <Tooltip text={text}>
      <button className="w-4 h-4 rounded-full bg-white/[0.06] text-white/30 flex items-center justify-center hover:bg-white/[0.08] hover:text-white/50 flex-shrink-0 transition-colors">
        <HelpCircle size={10} />
      </button>
    </Tooltip>
  );
...
export function KPI({
  ...
  return (
    <div className={cn(
      'bg-white/[0.03] rounded-xl border border-white/[0.06] p-5',
      ...
      <div className="flex items-center gap-2 mb-1.5">
        ...
        <span className="text-[11px] font-semibold text-white/30 uppercase tracking-wider">{title}</span>
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
      ...
      <p className="text-sm text-white/40 max-w-md mb-8 leading-relaxed">{description}</p>
      {action && (
        <button
          onClick={onAction}
          className={cn(
            'px-6 py-3 rounded-xl text-sm font-semibold shadow-sm transition-all duration-300',
            'bg-gradient-to-r from-[#833AB4] via-[#E1306C] to-[#F77737]',
            'text-white hover:shadow-lg hover:shadow-[#E1306C]/20 hover:scale-[1.02]'
          )}
        >
          {action}
        </button>
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 h-1.5 bg-white/[0.06] rounded-full overflow-hidden">
        <div className={cn('h-full rounded-full transition-all', c)} style={{ width: `${p}%` }} />
      </div>
      <span className="text-[11px] text-white/40 w-10 text-right tabular-nums">{v.toFixed(1)}</span>
    </div>
  );
}
...
export function IntBar({ v }: { v: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[11px] text-white/40 w-10 text-right tabular-nums">{(v * 100).toFixed(0)}%</span>
    </div>
  );
}
    ...
    <div className="flex items-center gap-2">
      {steps.map((_, i) => (
        <div key={i} className="flex items-center gap-2">
          ...
          {i < steps.length - 1 && (
            <div className={cn('w-10 h-0.5 rounded-full', i < current ? 'bg-emerald-500' : 'bg-white/[0.08]')} />
          )}
        </div>
      ))}
    </div>
  ...
  return (
    <div className="rounded-xl p-5 border bg-white/[0.02] border-white/[0.06]">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h4 className="font-semibold text-white/90 mb-1 text-[14px]">{title}</h4>
