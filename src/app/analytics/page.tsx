  ...
  return (
    <div className="space-y-6 max-w-[1400px]">
      ...
      <div className="pt-2 pb-4">
        <h1 className="text-xl font-bold text-white/90 flex items-center gap-2.5 mb-3">
          <BarChart3 size={20} className="text-[#E1306C]" />
          Analytics
          <span className="inline-flex items-center gap-1.5 ml-2 px-2.5 py-1 rounded-full bg-[#E1306C]/5 border border-[#E1306C]/10">
            <span className="text-[9px] font-bold text-[#E1306C] uppercase tracking-wider">Instagram</span>
          </span>
        </h1>
        ...
        <div className="pt-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-3xl mx-auto">
            {[
              ...
              const Icon = f.icon;
              return (
                <div key={f.title} className="bg-white/[0.03] rounded-2xl border border-white/[0.06] p-5 text-center hover:bg-white/[0.04] hover:border-white/[0.10] transition-all duration-200">
          ...
          <div className="text-center mt-8">
            <p className="text-[10px] text-white/30 uppercase tracking-wider font-semibold mb-3">Tente pesquisar</p>
            <div className="flex items-center justify-center gap-2 flex-wrap">
              {['nike', 'nasa', 'natgeo', 'cristiano'].map((handle) => (
                <button
                  key={handle}
                  onClick={() => handleSearch(handle)}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-white/50 bg-white/[0.03] hover:bg-white/[0.04] hover:text-[#E1306C] border border-white/[0.06] hover:border-[#E1306C]/20 transition-all duration-200"
                >
                  @{handle}
                  <ArrowRight size={10} />
                </button>
